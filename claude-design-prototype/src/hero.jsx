...
function HeroCanvas() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const g1 = ctx.createRadialGradient(W * 0.25, H * 0.3, 0, W * 0.25, H * 0.3, W * 0.55);
...
const ROLES = ['AI Engineer.', 'Agent Builder.', 'Full-Stack Developer.', 'Founder.'];
...
function TypingRoles() {
  ...
  useEffect(() => {
    if (reduced) { setText(ROLES[idx]); return; }
  ...
  return (
    <span className="sub">
      ...
      <span className="roles">{text}<span className="caret" /></span>
    </span>
  );
}
      ...
      <div className="hero-inner">
        ...
        <div>
          ...
          <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <TypingRoles />
            <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--muted)', fontSize: 14, maxWidth: 60 + 'ch', lineHeight: 1.6 }}>
              <span style={{ color: 'var(--amber)' }}>{'>'}</span>{' '}
              Building AI-powered systems and shipping real-world products —
              from concept to production. Currently founding{' '}
              <span style={{ color: 'var(--text)' }}>Appnado IT Solutions</span>.
            </div>
          </div>
        </div>
        ...
        <div className="hero-bottom">
          <div className="stat">
            <span className="v"><CountUp to={4} suffix="+" /></span>
            YEARS SHIPPING
          </div>
          <div className="stat">
            <span className="v"><CountUp to={24} /></span>
            PROJECTS DELIVERED
          </div>
          <div className="stat">
            <span className="v"><CountUp to={6} /></span>
            LANGUAGES IN ROTATION
          </div>
          <div className="stat">
            <span className="v" style={{ color: 'var(--amber)' }}>∞</span>
            CURIOSITY / DAY
          </div>
        </div>
      </div>
